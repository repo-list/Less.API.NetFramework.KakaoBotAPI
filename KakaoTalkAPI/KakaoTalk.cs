 ...
 * -> Developers using this automation API should never try to take any undesired actions which are opposite to the Kakao Terms of Service (http://www.kakao.com/policy/terms?type=ts).
            ...
            private bool _SendImageUsingClipboard(string imagePath, bool backupClipboardData)
            {
                ...
                try
                {
                    ...
                    lock (ChatWindows)
                    {
                        lock (Clipboard)
                        {
                            if (backupClipboardData) ClipboardManager.BackupData(this.RootHandle);
                            ClipboardManager.SetImage(imagePath, this.RootHandle);
                    ...
                    if (backupClipboardData) lock (Clipboard) { ClipboardManager.RestoreData(this.RootHandle); }
                }
            ...
            private Message[] _GetMessagesUsingClipboard(bool backupClipboardData)
            {
                ...
                lock (Clipboard)
                {
                    try
                    {
                        ...
                        messageString = ClipboardManager.GetText(this.RootHandle);
                        if (backupClipboardData) ClipboardManager.RestoreData(this.RootHandle);
                    }
