...
namespace SampleApplication
{
    class SampleQuizBot : QuizBot
    {
        ...
        public SampleQuizBot(string roomName, Target type, string identifier) : base(roomName, type, identifier) {}
        ...
        protected override void ParseMessage(string userName, string content, DateTime sendTime)
        {
            ...
            QuizUser user = (QuizUser)FindUserByNickname(userName);
            string[] words = GetOriginalCommand(content).Split(' ');

            switch (words[0])
            {
                case "!명령어": OnRequestingCommands(words); break;
                case "!정보": OnRequestingInfo(words, user); break;
                case "!퀴즈": OnRequestingQuiz(words); break;
                case "!종료": OnRequestingQuit(words); break;
                default: OnRequestingElse(words); return;
            }
        }

        private void OnRequestingCommands(string[] words)
        {
            string message = $"[명령어]\n";
            message += "!정보 : 유저 정보를 확인합니다.\n";
            message += "!퀴즈 : 퀴즈를 시작합니다.\n";

            Window.SendText(message);
            Thread.Sleep(SendMessageIntervalShort);
        }

        private void OnRequestingInfo(string[] words, QuizUser user)
        {
        ...
        private void OnRequestingQuiz(string[] words)
        {
            Window.SendText("퀴즈를 요청하셨습니다.");
            Thread.Sleep(SendMessageIntervalShort);
        }

        private void OnRequestingQuit(string[] words)
        {
            Window.SendText("종료를 요청하셨습니다.");
            Thread.Sleep(SendMessageIntervalShort);
            StopTask();
        }

        private void OnRequestingElse(string[] words) {}
    }
}
